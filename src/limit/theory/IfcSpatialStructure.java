...
public class IfcSpatialStructure
{
	...
	private void buildSpatialStructure()
	{
		...
		SET<IfcRelAggregates> decomposedBy = project.getIsDecomposedBy_Inverse();
		if (decomposedBy != null)
		{
			for (IfcRelAggregates subChild : decomposedBy)
			{
	...
	private void buildSpatialStructure(IfcObjectDefinition objDef, DefaultMutableTreeNode parent)
	{
		...
		{
			for (IfcRelAggregates relAggregates : objDef.getIsDecomposedBy_Inverse())
			{
			...
			for (String key : typeMap.keySet())
			{
				...
				for (IfcRoot ifcRoot : productMap.get(key))
				{
					...
					if(ifcRoot instanceof IfcObjectDefinition)
					{
						...
						{
							for (IfcRelAggregates ifcRelDecomposes : ifcObjectDefinition.getIsDecomposedBy_Inverse())
							{
